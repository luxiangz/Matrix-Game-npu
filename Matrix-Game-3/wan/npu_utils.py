...
ATTN_BACKEND_NPU_FA = "npu_fa"     # Ascend npu_fusion_attention (torch_npu native)
...
def _detect_attention_backend() -> str:
    ...
    if device_type == "npu":
        # Ascend: npu_fusion_attention (原生) > mindiesd (vllm-omni) > SDPA
        try:
            import torch_npu
            if hasattr(torch_npu, 'npu_fusion_attention'):
                _ATTN_BACKEND = ATTN_BACKEND_NPU_FA
                _logger.info("Attention backend: npu_fusion_attention (torch_npu native)")
                return _ATTN_BACKEND
